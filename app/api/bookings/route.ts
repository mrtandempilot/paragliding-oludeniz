...
async function getGoogleCalendarAccessToken(): Promise<string | null> {
  ...
  // Node's OpenSSL 3 binding (Vercel's runtime) can throw
  // "DECODER routines::unsupported" when a PEM string is handed straight to
  // Sign.sign() — parsing it into a KeyObject first via createPrivateKey()
  // sidesteps that and also tolerates \n vs literal "\n" and stray whitespace.
  const privateKeyObject = crypto.createPrivateKey(privateKeyRaw.replace(/\\n/g, '\n'))

  const signer = crypto.createSign('RSA-SHA256')
    signer.update(unsigned)
    signer.end()
    const jwt = `${unsigned}.${base64url(signer.sign(privateKeyObject))}`
